...
- 2026-09-15 · Matheus Lohse · TESTE HUMANO TASK-2-006 falhou (feedback dos catálogos): tela em tabela com formulário lateral fixo não atende; adição, edição e histórico devem abrir em MODAL a partir de uma lista → corrigido na SKIP `0.0.173`: painel refeito em lista de cartões com botão "Adicionar Evento/Natureza", Editar, Histórico e Inativar/Reativar abrindo modais. QA completo passou; aguarda reteste humano.

## 2026-09-16

- 2026-09-16 · Matheus Lohse · DEBUG TASK-2-006: teste humano reportou "Erro ao inativar natureza", "Erro ao criar natureza" e falha ao editar evento → causa raiz provada nos logs do backend: (1) `ReferenceError: __spreadValues is not defined` — spread de objeto não suportado no JSVM do PocketBase na resposta do hook; (2) `ativo: cannot be blank` — bool `required: true` rejeita `false` como vazio, bloqueando qualquer inativação. Corrigido na SKIP `0.0.174`: resposta do hook construída sem spread e migração `0059` tornou `ativo` opcional nas duas coleções. QA completo passou; migração aplicada; 401 sem auth reconfirmado. Aguarda reteste humano.
- 2026-09-16 · Matheus Lohse · DEBUG TASK-2-006: inativar uma Natureza global alterava retroativamente a associação exibida em regras da v2 Retirada, sem log no Rulebook → causa raiz: catálogos globais mutáveis ligados por relações vivas às regras históricas → corrigido na SKIP `0.0.175`: migração 0060 isolou Eventos/Naturezas e históricos por `rulebook_version_id`; somente Rascunhos aceitam alteração; versões Aprovadas/Retiradas são somente leitura; cópia duplica catálogos/regras/Golden Set e o modal permite somente copiar ou copiar + retirar origem com data final, tudo atômico e auditado no Rulebook. QA, schema, migração, logs e preview passaram; aguarda reteste humano.
